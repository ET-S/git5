...
from plot_config import arrow_active_color, arrow_alternative_color, arrow_inactive_color, \
    arrow_positive_color, arrow_negative_color, arrow_unknown_color
import numpy
node_names =['client_name', 
    'client_max_price', 
    'client_preference', 
    'product_id', 
    'product_category', 
    ...
    'cart_price', 
    'p2_view_product', 
    'p3_add_product_decision', 
    'p4_view_card', 
    'p5_buy_product_decision',
    'product_decision',
    'action_desicion']
var_positions = {
    # ALTITUDE
    ...
    #mine
    # 'client_name': [0,0], 
    # 'client_max_price': [0.3, 0.4], 
    # 'client_preference': [0.3, 0.2], 
    # 'product_id': [0.8, 0], 
    # 'product_category': [0.5, 0.18], 
    # 'product_liked': [0.7, 0.2], 
    # 'product_price': [0.8, 0.8], 
    # 'cart': [0.4, 0.62], 
    # 'cart_price': [0.6, 0.8], 
    # 'p2_view_product': [0.9, 0.4], 
    # 'p3_add_product_decision': [0.7, 0.38], 
    # 'p4_view_card': [0.6, 0.6], 
    # 'p5_buy_product_decision': [0.2, 0.8],
    # 'product_decision': [0.5, 0.38],
    # 'action_desicion': [0.2, 0.6],
...
def plot_from_adj_mat(
    ...
    adj_mat1 = adj_mat.copy()
    adj_mat2 = adj_mat.copy()
    for index, value in enumerate(adj_mat):
        for index2,value2 in enumerate(value):
            if value2 <= 1:
                adj_mat2[index,index2] = 0
            if value2 == 2:
                adj_mat1[index,index2] = 0
                adj_mat2[index,index2] = 0.5
            if value2 > 2:
                adj_mat1[index,index2] = 0
                adj_mat2[index,index2] = value2 - 2
    ...
    for i0, v0 in enumerate(var_names):
        for i1, v1 in enumerate(var_names):
            ...
            draw_edge(p0, p1, e01, e10, circle_radius, active_color=(0,0.9,0.2), label0=label1, label1=label0, mode=edge_mode)
    ...
    for i0, v0 in enumerate(var_names):
        for i1, v1 in enumerate(var_names):
            ...
            label0 = None if edge_labels is None else edge_labels[i0][i1]
            label1 = None if edge_labels is None else edge_labels[i1][i0]
            draw_edge(p0, p1, e01, e10, circle_radius, active_color= (1,0,0) ,label0=label1, label1=label0, mode=edge_mode)
    ...
    for i, var_name in enumerate(var_names):
        ...
        #ax.legend([text],[node_names[i]])
        #plt.savefig("fixit/analysed_data/as.jpg")
        #ax.add_patch(text)
